...
class PlaylistArchiverGUI:
    ...
    def create_dashboard_tab(self):
        ...
        # Refresh button - use instance method
        refresh_btn = ttk.Button(search_frame, text="Refresh", command=self.refresh_playlists)
    ...
    def refresh_playlists(self):
        ...
        try:
            ...
            for p_id, title, created, last_update, etag in playlists:
                ...
                self.playlist_treeview.insert('', END, text=f"{title} (ID: {p_id})", 
                                        values=(created_dt, last_update_dt, etag[:8] if etag else ''))
            
            # Select first item if any exist
            if self.playlist_treeview.get_children():
                self.playlist_treeview.selection()[0]
    ...
    def select_playlist(self, event=None):
        ...
        if selection:
            for item in selection:
                ...
                # Update status bar
                self.status_var.set(f"Selected: {title} (ID: {p_id})")
                
                # Load playlist into frame
                self.load_playlist_videos(p_id)
    ...
    def load_playlist_videos(self, playlist_id):
        ...
        try:
            ...
            for item in items:
                ...
                self.video_treeview.insert('', END, text=f"{status_icon} {item[3][:30]}", 
    ...
    def create_playlist_tab(self):
        ...
        # Back button
        back_btn = ttk.Button(frame, text="<< Back to Dashboard", command=self.refresh_playlists)
...
def delete_playlist():
    ...
    if response == 'yes':
        try:
            ...
            refresh_playlists()  # Refresh playlist list (module-level, will use instance if bound)
        except Exception as e:
...
def export_playlist():
    ...
    try:
        yt.export_playlist(current_playlist_id)
